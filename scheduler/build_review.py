...
function renderCards(){
  ...
  days.filter(d=>d.date===selDate).forEach(d=>{
    ...
    const people=Math.max(5,...d.stops.map(r=>C[r].size25||C[r].people||5));
    const real25=d.stops.reduce((a,r)=>a+(C[r].real25||0),0);
    const nstore=d.stops.filter(r=>String(C[r].storage).toLowerCase().startsWith('y')).length;
    card.innerHTML=`<div class="chead">
      ...
      <span class="cpeople">${IC.users} ${people} people${d.joint?' · '+IC.link+' with '+d.joint:(d.stacked>1?' ×'+d.stacked+' crews':'')}</span>
    ...
    d.stops.forEach((r,i)=>{
      ...
      el.innerHTML=`<span class="num" style="background:${col}">${i+1}</span>
        <div class="body"><div class="nm">${c.name}
          ...
          ${String(c.storage).toLowerCase().startsWith('y')?`<span class="badge store">${IC.box} ${c.boxes||'Y'}</span>`:''}
          ${approx?'<span class="badge approx">approx pin</span>':''}</div>
        <div class="sub">${c.zone} · 2026: <b>${isHalf?(c.h26/2).toFixed(1):c.h26}h</b> · 2025 real: ${c.real25!=null?c.real25.toFixed(1)+'h':'—'} · ${c.crew25||''}</div></div>
    ...
    if(d.note) card.insertAdjacentHTML('beforeend',`<div class="note">${d.note}</div>`);
