...
Importable: `run(sched)` evaluates every check against an in-memory
schedule dict and returns `(ok, warn)`; `report_lines(ok, warn)` renders
the exact text the script prints. Importing this module reads no files --
only running it as a script loads `cache/schedule.json`.
"""
...
def run(d, config=None):
    """Run every check against schedule dict `d` (the cache/schedule.json
    shape). Returns `(ok, warn)`: lists of `(status, msg)` tuples, in check
    order. `config` stands in for client_config_loader.load() (R5); when
    omitted it is loaded exactly where the script always loaded it.

    `rules` is imported here rather than at module level because importing
    it pulls in schedule.py, which reads client_config.json.
    """
    import rules
    ...
    scp_name = (config if config is not None else client_config_loader.load()
                )["single_crew_priority"]["client_name"]
...
def report_lines(ok, warn):
    """The script's stdout, one string per printed line."""
    lines = ["=" * 70]
    for status, msg in ok + warn:
        lines.append(f"[{status}] {msg}")
    lines.append("=" * 70)
    lines.append(f"{len(ok)} pass, {len(warn)} warn/fail")
    return lines


def main():
    # Same order as the old module-level script: rules (and with it
    # client_config.json) is imported before cache/schedule.json is read,
    # so a missing file fails the same way it always did.
    import rules  # noqa: F401
    d = json.load(open(os.path.join(CACHE, "schedule.json")))
    ok, warn = run(d)
    for line in report_lines(ok, warn):
        print(line)
